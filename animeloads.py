...
#                self.session.get(j.split("\"")[0])
    ...
    def downloadEpisode(self, episode, release, hoster, browser, browserlocation="", jdhost="", myjd_user="", myjd_pw="", myjd_device="", pkgName="", destinationFolder=None):
        if(browser == "Firefox"):
...
#        print("Key wurde erfolgreich deobfuskiert")
        ...
        if(jdhost != "" and myjd_user == ""):
            ...
            try:
                 myjd_return = utils.addToMYJD(myjd_user, myjd_pw, myjd_device, linkstring, pkgName, release.getPassword(), destinationFolder)
            except:
